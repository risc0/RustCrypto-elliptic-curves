...
#[cfg(all(target_os = "zkvm", target_arch = "riscv32"))]
use risc0_bigint2::{ec, field as Risc0Field};
...
pub trait PrimeCurveParams:
    ...
    ec::Curve<{Risc0Field::FIELD_384_WIDTH_WORDS}> + 
    Sized +
    PrimeCurve
    + CurveArithmetic
    + CurveArithmetic<AffinePoint = AffinePoint<Self>>
    + CurveArithmetic<ProjectivePoint = ProjectivePoint<Self>>
{
    ...
    const PRIME_LE_WORDS: [u32; Risc0Field::FIELD_384_WIDTH_WORDS];

    /// Order of the curve in little-endian words to be compatible with risc0 expected layout.
    const ORDER_LE_WORDS: [u32; Risc0Field::FIELD_384_WIDTH_WORDS];
    ...
    fn from_u32_words_le(words: [u32; Risc0Field::FIELD_384_WIDTH_WORDS]) -> Self::FieldElement;
}