...
pub struct FieldElement384<C> {
    pub data: [u32; FIELD_384_WIDTH_WORDS],
    _phantom: PhantomData<C>,
}

impl<C> Deref for FieldElement384<C> {
    type Target = [u32; FIELD_384_WIDTH_WORDS];

    fn deref(&self) -> &[u32; FIELD_384_WIDTH_WORDS] {
        &self.data
    }
}

impl<C: elliptic_curve::Curve> From<&FieldBytes<C>> for FieldElement384<C> {
    fn from(data: &FieldBytes<C>) -> Self {
        let mut words = [0u32; FIELD_384_WIDTH_WORDS];
...
impl<C: PrimeCurveParams> From<FieldElement384<C>> for GenericArray<u8, C::FieldBytesSize> {
...
impl<C> FieldElement384<C> {
    pub const fn new_unchecked(data: [u32; FIELD_384_WIDTH_WORDS]) -> Self {
        Self {
            data,
...
fn bytes_to_u32_words_le(bytes: &[u8]) -> [u32; FIELD_384_WIDTH_WORDS] {
    let mut words = [0u32; FIELD_384_WIDTH_WORDS];
...
pub(crate) fn felt_to_u32_words_le<C>(data: &C::FieldElement) -> [u32; FIELD_384_WIDTH_WORDS]
where
    C: PrimeCurveParams,
{
...
#[inline]
fn affine_to_r0_affine<C>(affine: &AffinePoint<C>) -> ec::AffinePoint<FIELD_384_WIDTH_WORDS, C>
where
    C: PrimeCurveParams,
{
...
pub(crate) fn projective_to_affine<C>(
    p: &ProjectivePoint<C>,
) -> ec::AffinePoint<FIELD_384_WIDTH_WORDS, C>
where
    C: PrimeCurveParams,
{
    let aff = p.to_affine();
...
pub(crate) fn affine_to_projective<C>(
    affine: &ec::AffinePoint<FIELD_384_WIDTH_WORDS, C>,
) -> ProjectivePoint<C>
where
    C: PrimeCurveParams,
{
...
pub(crate) fn scalar_to_words<C>(s: &Scalar<C>) -> [u32; FIELD_384_WIDTH_WORDS]
where
    C: PrimeCurveParams,
{
...
pub(crate) mod ec_impl {
        ...
        C: PrimeCurveParams,
    {
        ...
        let mut result = ec::AffinePoint::new_unchecked(
            [0u32; FIELD_384_WIDTH_WORDS],
            [0u32; FIELD_384_WIDTH_WORDS],
        );
